...
function loginrequired_init()
{
  ...
  //MIREM SI ES PETICION REST LLAVORS DEIXEM ACCES.
  
  $parameters_start = strrpos($current_url, 'auth.sign');
  if($parameters_start==0)$parameters_start = strrpos($current_url, 'user.auth');
  
  if($parameters_start != 0)return;
